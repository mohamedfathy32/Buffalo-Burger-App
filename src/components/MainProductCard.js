...
export default function MainProductCard({ prd = false }) {

    const product = prd === false ? {
        ...
        <TouchableOpacity>
            ...
            <View className="relative flex items-ed w-11/12 md:w-80 bg-neutral-200 rounded-xl shadow-lg p-4 mx-auto my-10">
                ...
                <View className="h-32 my-4">
                    <Text className="text-lg font-semibold uppercase">{product.title}</Text>
                    <Text className="text-neutral-400 my-2 line-clamp-2">{product.description}</Text>

                    <View className="flex-row justify-between items-center mt-auto">
                        <Text className="text-xl font-bold">{product.details?.size[0].price||product.price} EGP</Text>
