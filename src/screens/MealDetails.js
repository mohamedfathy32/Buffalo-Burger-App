...
export default function MealDetails({ navigation, route }) {
  ...
  useEffect(() => {
    const fetchDataAndSetValues = async () => {
      try {
        const breadsList = await fetchData("breads");
        const comboOptionsList = await fetchData("comboOptions");
        const drinksList = await fetchData("drinks");
        const extrasList = await fetchData("extras");

        setSizes(product?.details?.size);
        setBreads(breadsList);
        setComboOptions(comboOptionsList);
        setDrinks(drinksList);
        setExtras(extrasList);

        if (breadsList.length > 0) {
          setBread(breadsList[0].title.en);
        }
        if (comboOptionsList.length > 0) {
          setComboOption(comboOptionsList[0].title.en);
        }
        if (drinksList.length > 0) {
          setDrink(drinksList[0].title.en);
        }
        if (product?.details?.size?.length > 0) {
          setSize(product?.details?.size[0].title.en);
        }
      } catch (error) {
  ...
  const calculateTotalPrice = () => {
    ...
    const selectedComboOption = comboOptions.find(
      (c) => c.title.en === comboOption
    );
    const selectedDrink =
      comboOption !== "no combo"
        ? drinks.find((d) => d.title.en === drink)
        : { price: 0 };
    const extrasTotal = myExtras.reduce(
      (acc, me) => acc + (extras.find((e) => e.title.en === me)?.price || 0),
      0
    );
    return (
      (selectedSize?.price || 0) +
      (selectedBread?.price || 0) +
      (selectedComboOption?.price || 0) +
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <View className="flex-1 bg-white rounded-3xl relative">
        ...
        {/* Title & Description */}
        <Text className="font-bold text-3xl px-6 my-1 capitalize">
          {product.title.en}
        </Text>
        ...
        <ScrollView>
              ...
              <View className="flex flex-row justify-evenly">
                {sizes.map((s) => (
                  <TouchableOpacity
                    ...
                    className={`h-16 w-1/4 mx-3 flex items-center justify-center rounded-xl ${
                      s.title.en === size ? "bg-[#ff5f00]" : "bg-black"
                    }`}
                  >
          ...
          {product.details.bread && (
            <View className="h-auto bg-neutral-200 my-1">
              ...
              {breads.map((b) =>
                product.category === "chicken sandwiches" &&
                b.title.en === "lettuce bun" ? (
                  ""
                ) : (
                  <TouchableOpacity
                    key={b.title.en}
                    className={`${
                      b.title.en === bread ? "bg-[#ff5f00]" : "bg-white"
                    } w-11/12 mx-auto my-2 p-3 rounded-xl flex flex-row justify-between`}
                    onPress={() => {
                      setBread(b.title.en);
                    }}
                  >
                    <Text
                      className={`${
                        b.title.en === bread ? "text-white" : "text-black"
          ...
          {product.details.comboOptions && (
            <View className="h-auto bg-neutral-200 my-1">
              <Text className="text-center font-bold text-lg my-4">
                Combo Options
              </Text>
              {comboOptions.map((co) =>
                co.title.en === "no combo" ? (
                  ""
                ) : (
                  <TouchableOpacity
                    key={co.title.en}
                    onPress={() => {
                      comboOption === co.title.en
                        ? setComboOption("no combo")
                        : setComboOption(co.title.en);
                    }}
                  >
                    <View
                      className={`${
                        co.title.en === comboOption
                          ? "bg-orange-100 border-[#ff5f00]"
                          : "bg-white border-transparent"
                      } border-2 w-11/12 md:w-80 rounded-xl shadow-lg p-2 mx-auto my-2 flex flex-row `}
                    >
          ...
          {product.details.drinks && comboOption !== "no combo" && (
            <View className="h-auto bg-neutral-200 my-1">
              ...
              {drinks.map((d) => (
                ...
                >
                  <View
                    className={`w-11/12 bg-white md:w-80 rounded-xl shadow-lg p-2 mx-auto my-2 flex flex-row`}
                  >
                    <View className="w-2/3 flex flex-row">
                      <MaterialIcons
                        ...
                        style={{
                          marginHorizontal: 3,
                          color: d.title.en === drink ? "#f97316" : "black",
                        }}
                      />
                      <Text
                        className={`${
                          d.title.en === drink ? "text-orange-500" : ""
                        } mx-2 text-lg font-semibold capitalize`}
                      >
                        {d.title.en}
                      </Text>
                    </View>
                ...
                >
                  ...
                  >
                    <View className="w-2/3 flex flex-row">
                      <MaterialIcons
                        name={
                          myExtras.includes(extra.title.en)
                            ? "check-box"
                        ...
                        style={{
                          marginHorizontal: 3,
                          color: myExtras.includes(extra.title.en)
                            ? "#f97316"
                            : "black",
                        }}
                      />
        ...
        <TouchableOpacity
          ...
          className="bg-[#ff5f00] w-11/12 mx-auto my-6 p-3 rounded-xl flex flex-row justify-between"
        >
